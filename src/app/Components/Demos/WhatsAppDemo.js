...
function WhatsAppDemoBase() {
  const [messages, setMessages] = useState([
    {
      ...
      from: 'bot',
      text:
        "Bonjour 👋 Ici AetherIA. En 1 minute je comprends votre besoin, et on cale un appel de 15 min. Pour commencer, quel est votre enjeu principal ?",
    },
  ]);
  ...
  const botByStep = [
    () =>
      "Super, merci 👍. Pour situer un peu mieux, vous êtes plutôt dans le contexte d’une PME, d’une startup ou en indépendant ?",
    (a) =>
      `Parfait 🙌. L’idée c’est de vous donner rapidement 1 à 2 pistes concrètes adaptées à « ${a.enjeu} ».`,
    (a) =>
      `Top ✅ Voici le récap :
- Enjeu : « ${a.enjeu} »
- Contexte : « ${a.contexte} »
...
https://calendly.com/agenceaetheria/15min`,
  ...
  const choose = useCallback(
    async (reply) => {
      if (typing) return; // évite les clics multiples pendant la saisie
          ...
          setMessages([
            {
              ...
              from: 'bot',
              text:
                "Pas de souci 👍 On reprend depuis le début. Quel est votre enjeu principal ?",
            },
          ]);
          setStep(0);
      ...
      if (step === 1) {
        ...
        setMessages((m) => [
          ...m,
          {
            id: Date.now() + 3,
            from: 'bot',
            text: `Voici 3 créneaux courts (15 min) pour en parler 👇`,
          },
        ]);
        setTyping(false);
      }
    ...
    [step, answers, nextBotMessage, typing]
  );
  ...
  return (
    <div
      ...
      role="region"
      aria-label="Conversation simulée WhatsApp"
    >
        ...
        <AnimatePresence initial={false}>
          {messages.map((m) => (
            <motion.div
              ...
              className={`max-w-[85%] sm:max-w-[80%] px-3 py-2 rounded-lg ${
                m.from === 'bot'
      ...
      <div className="flex flex-wrap gap-2">
        {quickReplies.map((r) => (
          <button
            key={r}
            onClick={() => choose(r)}
            className="px-3 py-1.5 rounded-full border text-sm hover:bg-cyan-50/60 transition-colors disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/60 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--form-bg)]"
